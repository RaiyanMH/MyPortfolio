  ...
  return (
    ...
    >
      <div className="container-section flex items-center justify-between py-3">
        <Link href="#home" className="flex items-center gap-2 text-white/90">
        ...
        <nav className="flex items-center gap-1">
          {navItems.map((item) => (
            <Link
              key={item.id}
              href={item.href}
              className="relative px-3 py-2 rounded-full text-sm text-white/80 hover:text-white transition-colors"
            >
              {item.label}
              {active === item.id && (
                <motion.span
